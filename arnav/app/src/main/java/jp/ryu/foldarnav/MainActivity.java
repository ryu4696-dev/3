...
public class MainActivity extends Activity implements LocationListener, SensorEventListener {
    ...
    private ArNavigationView arView;
    ...
    private void openMapsAdjacent() {
        ...
        if (i == null) {
            i = new Intent(Intent.ACTION_VIEW, Uri.parse("geo:0,0?q="));
        }
        i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
                Intent.FLAG_ACTIVITY_LAUNCH_ADJACENT |
                Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
        try { startActivity(i); }
        catch (Exception e) { Toast.makeText(this, "Googleマップを開けません", Toast.LENGTH_SHORT).show(); }
    }
    ...
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grants) {
        ...
        if (requestCode == REQ_PERMS) {
            ...
            else Toast.makeText(this, "カメラと位置情報が必要です", Toast.LENGTH_LONG).show();
        }
    }
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode != RESULT_OK || data == null) {
            Toast.makeText(this, "画面共有が許可されていません", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void startArUi() {
        try {
            if (ArCoreApk.getInstance().requestInstall(this, true) ==
                ...
                Toast.makeText(this, "Google Play 開発者サービス（AR）の導入後、もう一度開始してください", Toast.LENGTH_LONG).show();
                return;
            }
        } catch (UnavailableException e) {
            Toast.makeText(this, "ARCoreを利用できません: " + e.getClass().getSimpleName(), Toast.LENGTH_LONG).show();
            return;
        }

        arStarted = true;
        arView = new ArNavigationView(this);
    ...
    @Override protected void onResume() {
        super.onResume();
        if (arStarted && arView != null) arView.resumeAr();
    }

    @Override protected void onPause() {
        if (arView != null) arView.pauseAr();
        super.onPause();
    }
    ...
    @Override protected void onDestroy() {
        ...
        if (isFinishing()) stopService(new Intent(this, ScreenCaptureService.class));
        super.onDestroy();
    }
